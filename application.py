...
def save_image_names_to_text_files():
    ...
    for fn_img in get_img_files(parsed.data):
        ...
        # plt.show()  to show the detections
...
reconstructed_model = keras.models.load_model("./ocr_model_50_epoch.h5", custom_objects=custom_objects,compile=False)
...
def copy_to_txt(content, filename):
    try:
        # Open the file in write mode (creates the file if it doesn't exist)
        with open(filename, 'w') as file:
            # Write the content to the file
            file.write(content)
        print(f"Content successfully copied to {filename}")
    except Exception as e:
        print(f"An error occurred: {e}")

file_name = "output.txt"
copy_to_txt(sentence, file_name)
print(sentence)